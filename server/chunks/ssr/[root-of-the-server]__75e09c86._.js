...
__turbopack_context__.s({
    ...
    "getPendingVerificationByToken": (()=>getPendingVerificationByToken),
...
async function appendUserToSheet(userData) {
    ...
    const request = {
        ...
        range: `${CREDENTIALS_SHEET_NAME}!A:H`,
...
async function getAllUsers() {
    try {
        ...
        // If there are no rows or only a header row, there are no users.
        ...
        if (userNameIndex === -1 || pwdIndex === -1 || emailIndex === -1 || databaseIdIndex === -1 || folderIdIndex === -1) {
            console.error("Credentials sheet is missing one of the required headers: emailAddress, userName, pwd, databaseID, folderID");
            throw new Error('Could not find required columns in the credentials sheet. Please check the sheet headers.');
        }
        ...
        console.error('The API returned an error when calling getAllUsers: ' + err.message);
        throw new Error('Failed to retrieve data from Google Sheet. Please ensure the Sheet ID is correct and that the service account has permission.');
    }
}
...
async function findRowIndexByToken(token, sheetName) {
    ...
    const tokenColumnIndex = 0; // Since we are only fetching column B
    const rowIndex = rows.findIndex((row)=>row[tokenColumnIndex] === token);
    if (rowIndex === -1) {
        // Fallback for pending verifications sheet
        const pendingResponse = await sheets.spreadsheets.values.get({
            spreadsheetId: SPREADSHEET_ID,
            range: `${PENDING_VERIFICATIONS_SHEET_NAME}!D:D`
        });
        const pendingRows = pendingResponse.data.values;
        if (!pendingRows) throw new Error('Could not find session or pending verification');
        const pendingRowIndex = pendingRows.findIndex((row)=>row[0] === token);
        if (pendingRowIndex === -1) throw new Error('Token not found in sessions or pending verifications');
        const sheetId = await getSheetId(SPREADSHEET_ID, PENDING_VERIFICATIONS_SHEET_NAME);
        if (sheetId === null) throw new Error(`Could not find sheet ID for ${PENDING_VERIFICATIONS_SHEET_NAME}.`);
        return {
            sheetId,
...
// ============== PendingVerifications Sheet Functions ==============
const PendingVerificationSchema = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["z"].object({
    ...
    expiration: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["z"].string()
});
async function addPendingVerification(data) {
    const sheets = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$drive$2d$oauth$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getSheetsClient"])();
    const sheetName = PENDING_VERIFICATIONS_SHEET_NAME;
    const range = `${sheetName}!A:E`;
    // Check for existing verification for this email and delete it
    const getResponse = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range
    });
    ...
    if (rows && rows.length > 0) {
        ...
        if (emailIndex !== -1) {
            const existingRowIndex = rows.findIndex((row)=>row[emailIndex] === data.email);
            if (existingRowIndex > 0) {
                ...
                console.log(`[Verification] Removed existing pending verification for ${data.email}.`);
            }
        }
    }
...
async function getPendingVerificationByToken(token) {
    const sheets = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$drive$2d$oauth$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getSheetsClient"])();
    const response = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
    ...
    const verificationRow = rows.slice(1).find((row)=>row[tokenIndex] === token);
    if (!verificationRow) return null;
    const expirationDate = new Date(verificationRow[expirationIndex]);
    ...
    return {
        email: verificationRow[header.indexOf('email')],
        userName: verificationRow[header.indexOf('userName')],
        password: verificationRow[header.indexOf('password')],
        token: verificationRow[tokenIndex],
        expiration: verificationRow[expirationIndex]
    };
}
async function deletePendingVerification(token) {
    ...
    console.log(`[Verification] Deleted pending verification with token ending in ...${token.slice(-4)}`);
}
}}),
...
var { g: global, __dirname } = __turbopack_context__;
{
/* __next_internal_action_entry_do_not_use__ [{"00186ae8b46ba208c20a4a809212c1f3e6948fe1b2":"getInitialPageTokenForDrive","40b059e63060f3f4feea26611d7aec8c3ec0ede92f":"downloadFileFromDrive","40dd2f09e524554b4ec3a9fff80a021fad2531abc0":"listDatFiles","40eb3992284d3f345830502ed25bef9bf64111ffac":"deleteFileFromDrive","6019d8efc0e8739c514bfb3847f6eef6107aca4d72":"createFolderInDrive","60f30c24add5ece81b8a4e4f22fb29dca30abc29de":"listChangesSincePageToken","70864395bb092dc39044827f4dcf80a5be65f97fdf":"checkFileExists","780db5225566d190dce92c616480c03104d3ffae89":"createSpreadsheetInFolder","7cada598f594d88c11337b2248ef7320b7b540f236":"uploadFileToDrive"},"",""] */ __turbopack_context__.s({
...
async function createSpreadsheetInFolder(name, parentFolderId, initialSheetName, headers) {
    const sheets = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$drive$2d$oauth$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getSheetsClient"])();
    const drive = await getDriveClient();
    ...
    console.log(`[Sheets] Created new spreadsheet with ID: ${spreadsheetId}`);
    // 2. Add headers to the first sheet
    await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: `${initialSheetName}!A1`,
        valueInputOption: 'RAW',
        requestBody: {
            values: [
                headers
    ...
    console.log(`[Sheets] Added headers to "${initialSheetName}".`);
    // 3. Move the spreadsheet to the specified folder
    ...
    console.log(`[Drive] Moved spreadsheet "${name}" to folder ID ${parentFolderId}.`);
...
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(createSpreadsheetInFolder, "780db5225566d190dce92c616480c03104d3ffae89", null);
...
"[project]/src/lib/nodemailer.ts [app-rsc] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
if (!EMAIL_SERVER_USER || !EMAIL_SERVER_PASSWORD) {
    console.warn('Email credentials (EMAIL_SERVER_USER, EMAIL_SERVER_PASSWORD) are not fully configured in .env. Email sending will be disabled.');
}
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$nodemailer$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/nodemailer.ts [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$action$2d$validate$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/dist/build/webpack/loaders/next-flight-loader/action-validate.js [app-rsc] (ecmascript)");
;
;
;
...
async function signUpUser(userData) {
    try {
        ...
        if (users.some((u)=>u.emailAddress === userData.email)) {
            return {
                success: false,
        ...
        const verificationUrl = `${("TURBOPACK compile-time value", "http://localhost:9002")}/verify?token=${token}`;
        await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$nodemailer$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["transporter"].sendMail({
            from: process.env.EMAIL_FROM,
            to: userData.email,
            subject: 'Verify Your DATGenie Account',
            html: `
        <h1>Welcome to DATGenie!</h1>
        <p>Please click the link below to verify your email address and activate your account. This link will expire in 1 hour.</p>
        <a href="${verificationUrl}" style="background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Verify Your Account</a>
...
async function verifyUserAccount(token) {
    try {
        // 1. Get pending verification data
        const pendingUser = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$googlesheets$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getPendingVerificationByToken"])(token);
        if (!pendingUser) {
            return {
                success: false,
        ...
        const rootFolderId = process.env.GOOGLE_DRIVE_ACCOUNTS_FOLDER_ID;
        if (!rootFolderId) throw new Error('Root folder ID is not configured.');
        const userFolderName = `DATGenie_${pendingUser.email}`;
        const userFolderId = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$drive$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["createFolderInDrive"])(userFolderName, rootFolderId);
        // 3. Create Google Sheet database
        const dbName = `Database - ${pendingUser.email}`;
        const tpListHeaders = [
            'tpTIN',
            'branchCode',
            'rdoCode',
        ...
        const databaseId = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$drive$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["createSpreadsheetInFolder"])(dbName, userFolderId, 'tpList', tpListHeaders);
        // 4. Append user to credentials sheet
        ...
        // 5. Delete pending verification record
    ...
    try {
        ...
        if (user) {
            ...
            if (!folderId) {
                ...
                const rootFolderId = process.env.GOOGLE_DRIVE_ACCOUNTS_FOLDER_ID;
                if (!rootFolderId) throw new Error('Root folder ID is not configured.');
                folderId = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$drive$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["createFolderInDrive"])(`DATGenie_${user.userName}`, rootFolderId);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__75e09c86._.js.map