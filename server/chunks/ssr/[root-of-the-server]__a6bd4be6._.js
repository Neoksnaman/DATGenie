...
var { g: global, __dirname } = __turbopack_context__;
{
/* __next_internal_action_entry_do_not_use__ [{"4019d8efc0e8739c514bfb3847f6eef6107aca4d72":"createFolderInDrive","40b059e63060f3f4feea26611d7aec8c3ec0ede92f":"downloadFileFromDrive","40dd2f09e524554b4ec3a9fff80a021fad2531abc0":"listDatFiles","70864395bb092dc39044827f4dcf80a5be65f97fdf":"checkFileExists","7cada598f594d88c11337b2248ef7320b7b540f236":"uploadFileToDrive"},"",""] */ __turbopack_context__.s({
...
async function processExcelFile(file, sheetName) {
    ...
    if (data.length <= 1) {
        return {
            data: [],
            validationErrors: [
                `Sheet "${sheetName}" has no data to process.`
            ]
        };
    }
    return {
...
async function generate1601EQSched1DatFile(file) {
    ...
    const validationErrors = [];
    const processedData = dataRows.filter((row)=>row.some((cell)=>String(cell).trim() !== '')) // Skip fully blank rows
    .map((row, index)=>{
        const originalRowNumber = index + 2;
        const processedRow = [
            ...row
    ...
    const datContent = processedData.map((row)=>row.slice(0, 10).join(',')).join('\n');
    return {
        ...defaultErrorResult,
        success: true,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a6bd4be6._.js.map