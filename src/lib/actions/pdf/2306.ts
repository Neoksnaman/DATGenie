...
async function fillPdfForm(
    ...
    const atcData = atcWF.find(item => item.atc === atcCode);
    form.getTextField('ATC').setText(atcCode);
    form.getTextField('FWTDescription').setText(atcData ? atcData.description : 'ATC NOT FOUND');
    
    form.getTextField('TotalPayment').setText(formatCurrency(rowData['TotalPayment']));
    form.getTextField('TotalFWT').setText(formatCurrency(rowData['TotalFWT']));
...
export async function generate2306Pdf(formData: FormData): Promise<PdfResult> {
    ...
    try {
        ...
        if (data.length <= 1) return { success: false, error: 'No data found in the "form_2306" sheet.' };
        ...
        for (const [index, row] of dataRows.entries()) {
            ...
            if (!atcCode) {
                ...
                rowData['ATC'] = atcCode;
                const atcData = atcWF.find(item => item.atc === atcCode);
                const excelRate = parseFloat(String(rowData['rate']));

                if (!atcData) {
                    validationErrors.push(`${errorPrefix}: ATC code '${atcCode}' is not a valid Final Withholding Tax code.`);
        ...
        const pdfUrl = `${protocol}://${host}/templates/form_2306_template.pdf`;
        ...
        if (collate === 'multiple') {
            ...
            for (const [index, pdfDoc] of pdfDocs.entries()) {
                ...
                const pdfFileName = `2306_${rowData['PayeeTIN']}_${sanitizedPayeeName}_${index + 1}.pdf`;
                zip.file(pdfFileName, pdfBytes);
            }
            const zipBuffer = await zip.generateAsync({ type: 'nodebuffer' });
            const base64 = zipBuffer.toString('base64');
            const fileName = `Generated_Certificates_2306_${Date.now()}.zip`;
            ...
            const fileName = `Generated_Certificates_2306_${Date.now()}.pdf`;
            return { success: true, base64, fileName };
        }

    } catch (error) {
        console.error('Error generating 2306 PDF:', error);
